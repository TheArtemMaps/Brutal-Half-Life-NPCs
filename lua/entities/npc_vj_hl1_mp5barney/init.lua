...
-----------------------------------------------*/
ENT.Model = {"models/hl1/barney.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 90
...
self.SoundTbl_MeleeAttackMiss = {"npc/zombie/claw_miss1.wav","npc/zombie/claw_miss2.wav"}
	...
	self:Give("weapon_vj_hlr1_mp5barn")
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnInitialize()
	...
	--if GetConVar("vj_pbhl_corpsedismemberment"):GetInt() == 1 then --uncomment this if needed (experimental, not finished)
	--self.DeathAnimationTime = 9999999999999999999999
	--self.WaitBeforeDeathTime = 9999999999999999999999
	--end
...
function ENT:Controller_IntMsg(ply, controlEnt)
	ply:ChatPrint("LALT: Holster / Unholster MP5")
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnThink(ply)
    ...
    for _, npc in pairs(ents.FindByClass("npc_*")) do
        if IsValid(npc) and npc:IsNPC() then
            ...
            for _, otherNPC in pairs(ents.FindByClass("npc_*")) do
                if IsValid(otherNPC) and otherNPC:IsNPC() and otherNPC ~= npc then
                    local otherNPCPos = otherNPC:GetPos()
                    local distance = npcPos:Distance(otherNPCPos)

                    if distance <= detectionRange then
                        -- The other NPC is within range, make the NPC attack it
...
function ENT:CustomOnTakeDamage_OnBleed(dmginfo, hitgroup, ply, pos, dir, intensity, damage)
