...
-----------------------------------------------*/
ENT.Model = {"models/hl1/shotgunbarney.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 90
...
self.SoundTbl_MeleeAttackMiss = {"npc/zombie/claw_miss1.wav","npc/zombie/claw_miss2.wav"}
	...
	self:Give("weapon_vj_hlr1_spas12barn")
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:Controller_IntMsg(ply, controlEnt)
	ply:ChatPrint("LALT: Holster / Unholster SPAS-12")
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnAcceptInput(key, activator, caller, data)
	...
	VJ_EmitSound(self, "items/gunpickup4.wav", 75, 100)
	elseif key == "draw1" then 
	VJ_EmitSound(self, "items/gunpickup2.wav", 75, 100)
	elseif key == "draw2" then
	VJ_EmitSound(self, "weapons2/scock1.wav", 75, 100)	
	end
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
if self.Security_UsingHurtWalk == true then
	...
	-- For guarding
	if self.IsGuard == true and self:GetWeaponState() == VJ_WEP_STATE_HOLSTERED and not IsValid(self:GetEnemy()) then
...
function ENT:CustomOnTakeDamage_BeforeImmuneChecks(dmginfo, hitgroup)
    ...
    if hitgroup == HITGROUP_GEAR then
        -- Check if the model is "models/hl1/shotgunotis.mdl"
    ...
    if self:GetModel() == "models/hl1/shotgunbarney2.mdl" then return end
...
return end
    ...
    if hitgroup == HITGROUP_LEFTLEG and self.totalDamage[hitgroup] > 9800 then
    ...
    elseif hitgroup == HITGROUP_RIGHTLEG and self.totalDamage[hitgroup] > 9800 then
    ...
    elseif hitgroup == HITGROUP_HEAD and dmginfo:GetDamageForce():Length() > 800 then
        ...
        local attacker = dmginfo:GetAttacker()
        self:TakeDamage(self:Health(), attacker, attacker)
    elseif hitgroup == HITGROUP_STOMACH and self.totalDamage[hitgroup] > 9800 then
        ...
        local attacker = dmginfo:GetAttacker()
        self:TakeDamage(self:Health(), attacker, attacker)
    elseif hitgroup == HITGROUP_LEFTARM and self.totalDamage[hitgroup] > 9800 then
        ...
        local attacker = dmginfo:GetAttacker()
        self:TakeDamage(self:Health(), attacker, attacker)
    elseif hitgroup == HITGROUP_RIGHTARM and self.totalDamage[hitgroup] > 9800 then
