...
-----------------------------------------------*/
ENT.Model = {"models/hl1/houndeye.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 80
...
function ENT:CustomOnThink()
    ...
    if self.VJ_IsBeingControlled then
        ...
        elseif not self.Houndeye_Sleeping and self.Houndeye_CurIdleAnim ~= 0 then
    ...
    if (self:GetMaxHealth() * 0.35) > self:Health() then
        if not self.Houndeye_LimpWalking then
    ...
    -- Blinking
    if not self.Dead and CurTime() > self.Houndeye_BlinkingT and not self.Houndeye_Sleeping then
        self:SetSkin(1)
...
---------------------------------------------------------------------------------------------------------------------------------------------
local houndeyeClasses = {npc_vj_pbhl_houndeye=true, npc_vj_hlr1a_houndeye=true}
--
...
function ENT:CustomOnFlinch_BeforeFlinch(dmginfo, hitgroup)
	...
	return self.CurAttackAnimTime and self.CurAttackAnimTime < CurTime()
end
...
function ENT:SetUpGibesOnDeath(dmginfo, hitgroup)
	...
	if self.HasGibDeathParticles then
		...
		util.Effect("bloodspray", effectData)
	        ...
	        self:CreateGibEntity("obj_vj_gib", "models/gib/agibs5.mdl", { BloodDecal = "VJ_HLR_Blood_Yellow", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
        self:SetBodygroup(4, 1)
		    self:CreateGibEntity("obj_vj_gib", "models/gib/agibs3.mdl", { BloodDecal = "VJ_HLR_Blood_Yellow", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})   
...
elseif hitgroup == HITGROUP_HEAD and self.totalDamage[hitgroup] > 800 then
    ...
    self:CreateGibEntity("obj_vj_gib", "models/gib/agibs3.mdl", { BloodDecal = "VJ_HLR_Blood_Yellow", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})   
	self:SetBodygroup(0, 1)
    elseif hitgroup == HITGROUP_CHEST and self.totalDamage[hitgroup] > 9500 then
    ...
    elseif hitgroup == HITGROUP_LEFTARM and self.totalDamage[hitgroup] > 9500 then
        ...
        util.Effect("bloodspray", effectData)
        VJ_EmitSound(self, "gore/amputation_light" .. math.random(1, 2) .. ".wav", 75, 100)
        self:CreateGibEntity("obj_vj_gib", "models/gib/agibs4.mdl", { BloodDecal = "VJ_HLR_Blood_Yellow", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
        self:SetBodygroup(2, 1)
        local attacker = dmginfo:GetAttacker()
        self:TakeDamage(self:Health(), attacker, attacker)
