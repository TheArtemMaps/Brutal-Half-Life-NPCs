...
-----------------------------------------------*/
ENT.Model = {"models/hl1/hgrunt.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 90
...
function ENT:CustomOnInitialize()
	...
	if myMDL == "models/hl1/hgrunt.mdl" then // Already the default
		self.HECU_Type = 0
		self.HECU_WepBG = 1
...
function ENT:CustomOnThink()
	...
	if self.HGrunt_LastBodyGroup != bgroup then
		...
		if self.HECU_Type == 0 then -- 0 = HL1 Grunt
			if bgroup == 0 then -- MP5
				self:DoChangeWeapon("weapon_vj_hlr1_mp5barn")
			elseif bgroup == 1 then -- Shotgun
				self:DoChangeWeapon("weapon_vj_hlr1_spas12barn")
...
function ENT:SetUpGibesOnDeath(dmginfo, hitgroup)
	...
	if self.HECU_Type == 0 && hitgroup == HITGROUP_HEAD then
		...
		if self.HasGibDeathParticles == true then
			...
			util.Effect("bloodspray", effectData)
		        ...
		        self:SetBodygroup(4, 1)
				        self:CreateGibEntity("obj_vj_gib", "models/gib/hgibs1.mdl", { BloodDecal = "VJ_HLR_Blood_Red", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		...
		self:CreateGibEntity("obj_vj_gib", "models/gib/hgibs11.mdl", { BloodDecal = "VJ_HLR_Blood_Red", Pos = self:LocalToWorld(Vector(0, 0, 40)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		...
		self:CreateGibEntity("obj_vj_gib","models/vj_hlr/gibs/flesh2.mdl",{BloodDecal="VJ_HLR_Blood_Red",Pos=self:LocalToWorld(Vector(0,1,40)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		self:CreateGibEntity("obj_vj_gib","models/vj_hlr/gibs/flesh3.mdl",{BloodDecal="VJ_HLR_Blood_Red",Pos=self:LocalToWorld(Vector(1,0,40)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		self:CreateGibEntity("obj_vj_gib","models/vj_hlr/gibs/flesh4.mdl",{BloodDecal="VJ_HLR_Blood_Red",Pos=self:LocalToWorld(Vector(0,2,40)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		...
		self:CreateGibEntity("obj_vj_gib","models/vj_hlr/gibs/hgib_legbone.mdl",{BloodDecal="VJ_HLR_Blood_Red",Pos=self:LocalToWorld(Vector(0,0,15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		self:CreateGibEntity("obj_vj_gib", "models/gib/grunt_gibs3.mdl", { BloodDecal = "VJ_HLR_Blood_Red", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		self:CreateGibEntity("obj_vj_gib", "models/gib/grunt_gibs3.mdl", { BloodDecal = "VJ_HLR_Blood_Red", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		self:CreateGibEntity("obj_vj_gib","models/vj_hlr/gibs/gib_hgrunt.mdl",{BloodDecal="VJ_HLR_Blood_Red",Pos=self:LocalToWorld(Vector(0,0,15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
...
-- Don't drop the mag if it's not MP5...
if self:GetBodygroup(1) == 1 then
return
end	
-- Drop the mag when reloading
self:CreateGibEntity("obj_vj_gib", "models/mags/mp5mag.mdl", { BloodDecal = "", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "weapons2/magbounce.wav"}})
 end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:CustomOnTakeDamage_OnBleed(dmginfo, hitgroup, ply, pos, dir)
    ...
    elseif hitgroup == HITGROUP_HEAD and dmginfo:GetDamageForce():Length() > 800 then
        ...
        local attacker = dmginfo:GetAttacker()
        self:TakeDamage(self:Health(), attacker, attacker)
    elseif hitgroup == HITGROUP_STOMACH and self.totalDamage[hitgroup] > 30000 then
