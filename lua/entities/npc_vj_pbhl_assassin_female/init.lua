...
-----------------------------------------------*/
ENT.Model = {"models/hl1/hassassin.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 60
...
function ENT:CustomOnThink()
	...
	elseif bGroup == 1 then
		self:DoChangeWeapon("weapon_vj_hlr1_glock17_sup2")
	end
...
function ENT:CustomOnTakeDamage_OnBleed(dmginfo, hitgroup, ply, pos, dir)
    ...
    elseif hitgroup == HITGROUP_HEAD and dmginfo:GetDamageForce():Length() > 800 then
        ...
        util.Effect("bloodspray", effectData)
        VJ_EmitSound(self, "gore/amputation_light" .. math.random(1, 2) .. ".wav", 75, 100)
        self:CreateGibEntity("obj_vj_gib", "models/vj_hlr/gibs/hgib_skull.mdl", { BloodDecal = "VJ_HLR_Blood_Red", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
		self:CreateGibEntity("obj_vj_gib", "models/gibs/humans/brain_gib.mdl", { BloodDecal = "VJ_HLR_Blood_Red", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
        self:SetBodygroup(0, 1)
		self:SetBodygroup(7, 1)
        local attacker = dmginfo:GetAttacker()
    ...
    elseif hitgroup == HITGROUP_LEFTARM and self.totalDamage[hitgroup] > 35000 then
        ...
        util.Effect("bloodspray", effectData)
        VJ_EmitSound(self, "gore/amputation_light" .. math.random(1, 2) .. ".wav", 75, 100)
        self:CreateGibEntity("obj_vj_gib", "models/vj_hlr/gibs/hgib_b_bone.mdl", { BloodDecal = "VJ_HLR_Blood_Red", Pos = self:LocalToWorld(Vector(0, 0, 15)),CollideSound={ "gore/flesh" .. math.random(1, 3) .. ".wav"}})
        self:SetBodygroup(2, 1)
        local attacker = dmginfo:GetAttacker()
        self:TakeDamage(self:Health(), attacker, attacker)
