...
-----------------------------------------------*/
ENT.Model = {"models/hl1/sniperotis.mdl"} -- The game will pick a random model from the table when the SNPC is spawned | Add as many as you want
ENT.StartHealth = 90
...
if GetConVar("russian_voice"):GetInt() == 1 then
	...
	self:Give("weapon_vj_hlr1_m40a1barn")
end
---------------------------------------------------------------------------------------------------------------------------------------------
...
function ENT:Controller_IntMsg(ply, controlEnt)
	ply:ChatPrint("LALT: Holster / Unholster M40A1")
end
---------------------------------------------------------------------------------------------------------------------------------------------
function ENT:CustomOnAcceptInput(key, activator, caller, data)
	...
	VJ_EmitSound(self, "vj_hlr/hl1_weapon/sniper/sniper_bolt1.wav", 75, 100)	
	end
end
...
function ENT:Security_HolsterGun()
	...
	timer.Simple(self.Security_Type == 2 and 1 or 0.9, function()
...
function ENT:Security_UnHolsterGun()
	...
	timer.Simple(0.5, function() if IsValid(self) then self:SetBodygroup(1, 1) end end)
end
---------------------------------------------------------------------------------------------------------------------------------------------
